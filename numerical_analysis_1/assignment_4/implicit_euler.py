...
def chebyshev_nodes(n):
    return np.array([math.cos((2*k - 1) * math.pi / (2 * n)) for k in range(1, n + 1)])
...
if __name__ == "__main__":
    ...
    x_1 = np.random.uniform(-2, 2, 10)
    x_2 = np.linspace(-2, 2, 20)
    x_nodes = np.concatenate((x_1, x_2))
    x_nodes = np.unique(x_nodes)
    x_nodes = np.sort(x_nodes)
    # x_nodes = np.linspace(-2, 2, 10)
    width = len(x_nodes)
    dt = 0.01
    t_final = 1
